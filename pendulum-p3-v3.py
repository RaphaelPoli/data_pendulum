...
while not ennd:
	...
	screen = pygame.display.set_mode((400,164))
	...
	while not exti:	
		...
		if len(data)>108: #minimum sample to get a frequency (first used 500)
			...
			if hires:
				...
				if d>switch:#when the number of seconds is attained (hires is lower number of seconds)
					...
					if len(freq_data)>5:
						...
						if len(wide_coef_data)>260:
							wide_coef_data.pop(0)
						
						ech_coef_inter=[]
						if len(wide_coef_data)>5:
						...
						print("coef inter:", mean(ech_coef_inter))
					...
					if counting:
						...
						if long_term_rec:
							row=[datetime.datetime.now(),mean(data),mean(freq_data),coef_inter, motion,ambitus,mean(wide_motion_data),mean(ambitus_data),coef_linear_model,amplitude, mean(band_width_data),mean(min_freq_data), mean(max_freq_data),vibrato[0], vibrato_blur]
							xls_add_row(vibration_xls, row)
			...
			if lowres:
				d=int(datetime.datetime.strftime(datetime.datetime.now(),"%s"))-int(datetime.datetime.strftime(time_place, "%s"))
				if d>10:#when the number of seconds is attained (lowres is a higher number of seconds)
					...
					row=[datetime.datetime.now(),mean(data),mean(freq_data),coef_linear_model,amplitude, maxcalm, blurry_count,mean(band_width_data),mean(min_freq_data), mean(max_freq_data),vibrato[0], vibrato_blur]
					blurry_count=0
					blind_add_row(row)
					time_place=datetime.datetime.now()
			...
			screen.blit(truth,(x-5 - truth.get_width(), 30-10 - truth.get_height() // 2))
			screen.blit(intensity,(x-5 - intensity.get_width(), 60-10 - intensity.get_height() // 2))
			screen.blit(intruth,(x-5 - intruth.get_width(), 90-10 - intensity.get_height() // 2))
			
			screen.blit(last_18_min,(x-5 - last_18_min.get_width(), 120-10 - last_18_min.get_height() // 2))
			...
			pygame.draw.line(screen,color, (x-2,30-10), (x+coef_dspl,30-10),20)
			pygame.draw.line(screen,(255,255,0), (x-2,60-10), (x+ambitus_display_deg_3,60-10),20)
			
			pygame.draw.line(screen,inter_color, (x-2,90-10), (x+inter_coef_dspl,90-10),20)
			
			pygame.draw.line(screen,color_mean, (x-2,120-10), (x+wide_coef_dspl,120-10),20)
